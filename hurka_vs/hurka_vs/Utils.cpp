...
/// \brief given "data\\fonts\\consola.ttf" this program should return
/// C:\github\hurkalumo\hurka_vs\debug\data\fonts\consola.ttf 
// (--)
std::string getFullUri(std::string relativeFilepath)
{ 
	...
	std::wstring fullpath_w = std::wstring(buffer).substr(0, pos); 
	std::string fullpath(fullpath_w.begin(), fullpath_w.end());
