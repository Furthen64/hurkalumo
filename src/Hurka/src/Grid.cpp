...
void Grid::draw( RenderTarget& rt, HPos *viewHPos)
{
    ...
    for(int M= 0; M<height; M++){
        for(int N= 0; N < width; N++) {
            ...
            gx = convert_iso_to_gpix_x_topleft(M,N, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT,0);
            gy = convert_iso_to_gpix_y_topleft(M,N, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT,0);


            wx = gx + viewHPos->gpix_x_topleft;
            wy = gy + viewHPos->gpix_y_topleft;
    ...
    gx = convert_iso_to_gpix_x_topleft(selected_iso_pos.y, selected_iso_pos.x, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT, 0);
    gy = convert_iso_to_gpix_y_topleft(selected_iso_pos.y, selected_iso_pos.x, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT, 0);
...
void Grid::setVisible(HPos *_pos)
{
    ...
    selected_pix_pos_delete.y = _pos->gpix_y_topleft;
    selected_pix_pos_delete.x = _pos->gpix_x_topleft;
}
                     ...
                     int debugLevel)
{
	...
	if(nrTiles <= 16) {
		return bruteForceFindTile( entireGameboard, relRect, searchpos_gpix_middle, ind, rt, viewHPos, recursionName , debugLevel);                // RECURSION END
	}
...
HPos *Grid::bruteForceFindTile(HRect *entireGameboard, HRect *relRect, HPos *searchpos_gpix_middle, std::string ind, RenderTarget &rt, HPos *viewHPos, std::string recursionName, int debugLevel)
{
    ...
    for(int Y = relRect->absStart->abs_iso_y; Y < (relRect->absStart->abs_iso_y+relRect->rows); Y++)
    {
        ...
        for(int X = relRect->absStart->abs_iso_x; X < (relRect->absStart->abs_iso_x+relRect->cols); X++)
        {
            ...
            currentY = Grid::convert_iso_to_gpix_y_topleft(isopos->abs_iso_y, isopos->abs_iso_x, GRID_TEXTURE_WIDTH, GRID_TEXTURE_HEIGHT, 0);
            ...
            if(debugLevel >=1 && debugDrawing)
            {
                ...
                currIsoPosRect.setPosition(sf::Vector2f( isopos->gpix_x_topleft + viewHPos->gpix_x_topleft, isopos->gpix_y_topleft + viewHPos->gpix_y_topleft));
            ...
            if( isopos->gpix_x_topleft <= searchPos_gpix_x && searchPos_gpix_x < (isopos->gpix_x_topleft + GRID_TEXTURE_WIDTH) ) {
            ...
            if( isopos->gpix_y_topleft <= searchPos_gpix_y && searchPos_gpix_y < (isopos->gpix_y_topleft + GRID_TEXTURE_HEIGHT) ) {
            ...
            xoffset = isopos->gpix_x_topleft;              // add the position of the Tile we're currently looking at
            ...
            for(int y = 0; y < (GRID_TEXTURE_HEIGHT/2); y++)
            {
                ...
                if(debugLevel >=1 && debugDrawing == true)
                {
                    ...
                    drawFromX = fromX + viewHPos->gpix_x_topleft;
                    drawFromY = currentY + viewHPos->gpix_y_topleft;
